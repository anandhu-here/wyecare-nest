...
export class PermissionGuard implements CanActivate {
  ...
  // List of system-level permissions that don't require organization context
  private readonly systemPermissions = [
    'create_organization',
    'manage_system_settings',
    // Add other system-level permissions here
  ];
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    // Split required permissions into system and organization types
    ...
    // If we only had system permissions and they all passed, we're done
    if (orgLevelPermissions.length === 0 && requireAny?.length === 0) {
      return true;
    }

    // For organization-level permissions, we need an organization context
    ...
    // Validate user's access to this organization
    const hasAccess =
